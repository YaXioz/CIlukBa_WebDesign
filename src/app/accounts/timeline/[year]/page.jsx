...
export default async function Page({ params }) {
  ...
  return (
    <div className="w-full flex bg-gradient-to-br from-[#1A0733] to-[#5E27D1] min-h-screen">
      <div className="mt-16 container flex flex-col md:flex-row lg:flex-row justify-between gap-8 mx-auto px-4 lg:px-12">
        <aside className="max-w-max">
          <Navside />
        </aside>
        <div className="bg-[#171D22] rounded-lg flex flex-col items-center w-full lg:w-2/3">
          ...
          {isEmpty(posts) ? (
            <div className="text-center w-full border-t border-[#d9d9d9] p-10 text-[#f5f5f5]">No post has been created</div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 border-t border-[#d9d9d9] p-5">
              {posts?.map((post, i) => (
                <Link href={`/accounts/post/edit/${params?.year}/${post?.url}`} key={i} className="transition-transform transform hover:scale-105">
                  <img
                    ...
                    className="w-full h-[240px] object-cover rounded-lg shadow-lg transition-transform duration-300 hover:shadow-2xl hover:contrast-75"
                    alt=""
                  />
                </Link>
              ))}
          ...
          <div className="absolute bottom-0 w-16 h-32 bg-[#171d22] left-1/2 transition-transform hover:scale-110 z-10 -translate-x-1/2">
            <div className="py-4 px-4 bg-gradient-to-t from-[#1A0733] to-[#6E27D1] rounded-full text-center shadow-lg flex flex-col items-center">
