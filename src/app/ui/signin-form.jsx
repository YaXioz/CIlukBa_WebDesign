  ...
  return (
    <div className="relative py-16 bg-gradient-to-br from-sky-50 to-gray-200 min-h-screen">
      ...
      <div class="flex justify-center mb-4"> <img src="/image/Logo Black.png" alt="Logo" class="h-12"/> </div> 
        ...
        <div className="rounded-xl bg-[#1a1a1a] shadow-xl">
          <div className="p-6 sm:p-16">
            
    <form action={action}>
      <div className="grid mb-6">
        <label htmlFor="email" label className="mb-4 text-white">Email</label>
        <input className="bg-gray-200 p-2 text-black rounded-lg" id="email" name="email" placeholder="john@example.com" />
      </div>
      {state?.errors?.email && <p>{state.errors.email}</p>}

      <div className="grid mb-6">
        <label htmlFor="password" label className="mb-4 text-white">Password</label>
    ...
    <div className="mt-5">
      <Link className="text-white" href="signin">Don't have an account?</Link>
    </div>
    </div>
    </div>
    </div>
  ...
  return (
    <div className="flex justify-center">
      <div class="b mx-auto h-16 w-64 flex justify-center items-center">
        ...
        <a class="text-center text-white font-semibold z-10 pointer-events-none">Sign In!</a>
      </div>
    </div>
  );
}
